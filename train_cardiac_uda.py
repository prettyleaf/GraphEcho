...
DEFAULT_CARDIAC_ROOT = os.environ.get("CARDIAC_UDA_ROOT", os.path.join(DATASETS_ROOT, "cardiacUDC_dataset"))
DEFAULT_CARDIAC_INFOS = os.environ.get("CARDIAC_UDA_INFOS", os.path.join(DATASETS_ROOT, "infos.npy"))
DEFAULT_CARDIAC_CYC_INFOS = os.environ.get("CARDIAC_UDA_CYC_INFOS", "")

# Default CardiacUDA dataset root.
root = DEFAULT_CARDIAC_ROOT
...
#                    }
#      }
if not os.path.isfile(DEFAULT_CARDIAC_INFOS):
    raise FileNotFoundError(
        f"infos.npy not found at {DEFAULT_CARDIAC_INFOS}. "
        f"Set CARDIAC_UDA_INFOS or place infos.npy under {DATASETS_ROOT}."
    )
infos = np.load(DEFAULT_CARDIAC_INFOS, allow_pickle=True).item()
...
class Trainer():
    def __init__(self, config, debug=False):
        ...
        self.dataset_root = data_cfg.get("cardiac_root", root)
        self.cyc_infos_path = data_cfg.get("cyc_infos_path", DEFAULT_CARDIAC_CYC_INFOS)
        ...
        if self.temporal_graph:
            self.train_dataset_source_temp = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=True, set_select=['gy'], view_num=self.view_num, seg_parts=self.seg_parts, single_frame=False)
            self.train_dataset_target_temp = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=True, set_select=['rmyy'], view_num=self.view_num, seg_parts=self.seg_parts, single_frame=False, repeat=2, source_domain=False)
        ...
        self.train_dataset_source = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=True, set_select=['Site_G'], view_num=self.view_num, seg_parts=self.seg_parts)
        self.train_dataset_target = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=True, set_select=['Site_R'], view_num=self.view_num, seg_parts=self.seg_parts, source_domain=False)
        ...
        if self.cyc_loss:
            if not self.cyc_infos_path:
                raise FileNotFoundError(
                    "cyc_loss=True but cyc infos path is not set. "
                    "Set config['data']['cyc_infos_path'] or CARDIAC_UDA_CYC_INFOS."
                )
            infos_cyc = np.load(self.cyc_infos_path, allow_pickle=True).item()
            self.train_cyc_dataset = Seg_Cardiac_UDA_Dataset(infos_cyc, self.dataset_root, is_train=True, set_select=['Site_G'], view_num=self.view_num, single_frame=False, clip_length=64)
            self.train_cyc_loader = DataLoader(self.train_cyc_dataset, batch_size=1, shuffle=True, num_workers=config['train']['num_workers'])
        
        self.valid_dataset = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=False, data_list=self.train_dataset_source.valid_list, set_select=['Site_R'], view_num=self.view_num, seg_parts=self.seg_parts)
        self.valid_loader = DataLoader(self.valid_dataset, batch_size=1, shuffle=False, num_workers=config['train']['num_workers'])
        
        self.test_dataset  = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=False, data_list=self.train_dataset_source.test_list, set_select=['Site_R'], view_num=self.view_num, seg_parts=self.seg_parts)
        self.test_loader  = DataLoader(self.test_dataset, batch_size=1, shuffle=False, num_workers=config['train']['num_workers'])

        self.test_target_dataset = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=False, data_list=self.train_dataset_target.test_list, set_select=['Site_R'], view_num=self.view_num, seg_parts=self.seg_parts)
        ...
        self.test_target_video_dataset = Seg_Cardiac_UDA_Dataset(infos, self.dataset_root, is_train=False, set_select=['Site_R_full'], view_num=self.view_num, seg_parts=self.seg_parts, single_frame=False, fill_mask=True)
...
if __name__ == "__main__":
    config = {
                ...
                "data":{
                        "cardiac_root": DEFAULT_CARDIAC_ROOT,
                        "cyc_infos_path": DEFAULT_CARDIAC_CYC_INFOS,
                        },
