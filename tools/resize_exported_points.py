...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override class id in output labels (e.g. 0).",
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Write normalized coordinates (x/width, y/height).",
    )
...
def normalize_points(
    ...
    return [(x / float(out_w), y / float(out_h)) for x, y in points]


def write_annotation(
    path: Path,
    polygons: Sequence[Tuple[str, Sequence[Tuple[float, float]]]],
    normalize: bool,
) -> None:
    with path.open("w", encoding="utf-8", newline="\n") as f:
        for cls, points in polygons:
            if normalize:
                ...
                coords = " ".join(f"{int(round(x))} {int(round(y))}" for x, y in points)
...
def process_sample(
    ...
    polygons_out: List[Tuple[str, Sequence[Tuple[float, float]]]] = []
    dropped = 0
    if label_in.exists():
        with label_in.open("r", encoding="utf-8") as f:
            for raw in f:
                ...
                if parsed is None:
                    continue
                cls, points = parsed
                if class_id_out is not None:
                    cls = str(class_id_out)
                ...
                if unique_point_count(new_points) < 3:
                    ...
                    continue
                if normalize:
                    polygons_out.append((cls, normalize_points(new_points, t.out_w, t.out_h)))
                else:
                    polygons_out.append((cls, [(float(x), float(y)) for x, y in new_points]))

    image_out.parent.mkdir(parents=True, exist_ok=True)
    label_out.parent.mkdir(parents=True, exist_ok=True)
    if not cv2.imwrite(str(image_out), image_resized):
        raise RuntimeError(f"Failed to write image: {image_out}")
    write_annotation(label_out, polygons_out, normalize=normalize)
    return True, dropped
    ...
    for ds_dir in ds_dirs:
        ...
        for image_in in image_files:
            ...
            wrote, dropped = process_sample(
                ...
                class_id_out=args.class_id_out,
