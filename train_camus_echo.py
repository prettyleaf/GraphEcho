...
DEFAULT_CAMUS_ROOT = os.environ.get("CAMUS_DATASET_ROOT", os.path.join(DATASETS_ROOT, "CAMUS_public"))
DEFAULT_ECHONET_ROOT = os.environ.get("ECHONET_DATASET_ROOT", os.path.join(DATASETS_ROOT, "EchoNet-Dynamic"))
...
class Trainer():
    def __init__(self, config, debug=False):
        ...
        self.camus_root = data_cfg.get("camus_root", DEFAULT_CAMUS_ROOT)
        self.echonet_root = data_cfg.get("echonet_root", DEFAULT_ECHONET_ROOT)

        if not os.path.isdir(self.camus_root):
            raise FileNotFoundError(
                f"CAMUS dataset path not found: {self.camus_root}. "
                f"Set config['data']['camus_root'] or CAMUS_DATASET_ROOT."
            )
        if not os.path.isdir(self.echonet_root):
            raise FileNotFoundError(
                f"EchoNet dataset path not found: {self.echonet_root}. "
                f"Set config['data']['echonet_root'] or ECHONET_DATASET_ROOT."
            )
        ...
        if self.temporal_graph:
            self.train_dataset_source_temp = DataLoaderCamus(dataset_path=self.camus_root,input_name="4CH_ED",target_name="4CH_ED",condition_name="4CH_ED_gt",stage="train", img_res=(124, 124), img_crop=(112, 112))
            self.train_dataset_target_temp = Echo(root=self.echonet_root, validation=False, split='train')
        ...
        self.train_dataset_source = DataLoaderCamus(dataset_path=self.camus_root,input_name="4CH_ED",target_name="4CH_ED",condition_name="4CH_ED_gt",stage="train", img_res=(124, 124), img_crop=(112, 112))
        self.train_dataset_target = Echo(root=self.echonet_root, validation=False, split='train')
        ...
        self.valid_echo_dataset  = Echo(root=self.echonet_root, validation=True, split='VAL', target_type='LargeTrace')
        ...
        self.test_echo_dataset  = Echo(root=self.echonet_root, validation=True, split='TEST', target_type='LargeTrace')
        self.test_echo_loader  = DataLoader(self.test_echo_dataset, batch_size=1, shuffle=False, num_workers=config['train']['num_workers'])
        
        self.valid_camus_dataset  = DataLoaderCamus(dataset_path=self.camus_root,input_name="4CH_ED",target_name="4CH_ED",condition_name="4CH_ED_gt",stage="valid", img_res=(124, 124), img_crop=(112, 112))
        ...
        self.test_camus_dataset  = DataLoaderCamus(dataset_path=self.camus_root,input_name="4CH_ED",target_name="4CH_ED",condition_name="4CH_ED_gt",stage="test", img_res=(124, 124), img_crop=(112, 112))
...
if __name__ == "__main__":
    config = {
                ...
                "data":{
                        "camus_root": DEFAULT_CAMUS_ROOT,
                        "echonet_root": DEFAULT_ECHONET_ROOT,
                        },
